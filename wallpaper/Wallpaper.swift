// Adapted from Desktop Habitats by Chase Lean (MIT); see THIRD_PARTY_NOTICES.md.
...
enum Habitat: String, CaseIterable {
  case reef
  var title: String { "深蓝珊瑚缸" }
  var page: String { "/app/index.html" }
  var background: NSColor { NSColor(calibratedRed: 0.008, green: 0.039, blue: 0.106, alpha: 1) }
...
final class SceneHandler: NSObject, WKURLSchemeHandler {
  ...
  func webView(_ webView: WKWebView, start task: WKURLSchemeTask) {
    guard let url = task.request.url, url.host == sceneHost else {
...
final class Reporter: NSObject, WKScriptMessageHandler {
  ...
  func userContentController(
    ...
    NSLog("deep-reef page: \(message.body)")
  }
}
...
final class DesktopWindow: NSWindow {
  ...
  override var canBecomeKey: Bool { previewMode }
  override var canBecomeMain: Bool { previewMode }
}

/// The host is the single owner of native pause and lighting preferences.
final class ReefControl: NSObject, WKScriptMessageHandler {
  static let shared = ReefControl()
  func userContentController(_ controller: WKUserContentController, didReceive message: WKScriptMessage) {
    guard message.frameInfo.isMainFrame,
      message.frameInfo.request.url?.scheme == sceneScheme,
      message.frameInfo.request.url?.host == sceneHost,
      let body = message.body as? [String: Any], let type = body["type"] as? String else { return }
    if type == "pause", let value = body["value"] as? Bool { Controller.shared?.setPaused(value) }
    if type == "light", let value = body["value"] as? String { Controller.shared?.setLight(value) }
    if type == "ready" { Controller.shared?.syncPages() }
  }
}
...
final class Wallpaper: NSObject, WKNavigationDelegate {
  ...
  init(screen: NSScreen, root: URL, habitat: Habitat) {
    ...
    settings.userContentController.add(ReefControl.shared, name: "reefControl")
    ...
    let frame = previewMode ? NSRect(x: screen.visibleFrame.midX - 640, y: screen.visibleFrame.midY - 360, width: 1280, height: 720) : screen.frame
    window = DesktopWindow(
      contentRect: frame, styleMask: previewMode ? [.titled, .closable, .miniaturizable, .resizable] : .borderless, backing: .buffered, defer: false,
      screen: screen)
    super.init()

    view.navigationDelegate = self
    window.title = "Deep Reef · 深蓝珊瑚缸预览"
    window.level = previewMode ? .normal : NSWindow.Level(rawValue: Int(CGWindowLevelForKey(.desktopWindow)))
    window.collectionBehavior = previewMode ? [.managed] : [.canJoinAllSpaces, .stationary, .ignoresCycle]
    window.ignoresMouseEvents = !previewMode
    ...
    if previewMode { window.makeKeyAndOrderFront(nil); NSApp.activate(ignoringOtherApps: true) }

    view.load(URLRequest(url: URL(string: "\(sceneScheme)://\(sceneHost)\(habitat.page)\(previewMode ? "?preview=1" : "")")!))
  }
  ...
  func close() {
    ...
    view.configuration.userContentController.removeScriptMessageHandler(forName: "reefControl")
  ...
  private func send() {
    ...
    view.evaluateJavaScript(
      """
      typeof habitatPaused === 'function' && habitatPaused(\(paused ? "true" : "false"));
  ...
  func reload() { loaded = false; view.reload() }

  func syncState() {
    loaded = true
    let light = UserDefaults.standard.string(forKey: "light") ?? "blue"
    if ["blue", "moon", "day"].contains(light) { view.evaluateJavaScript("window.habitatLight && habitatLight(\"\(light)\")") }
    send()
  }
  ...
  func webView(
    ...
    NSLog("deep-reef: the scene did not load: \(error.localizedDescription)")
  }
  ...
  func probe() {
    view.evaluateJavaScript(
      ...
      NSLog("deep-reef page state: \(value ?? error?.localizedDescription ?? "unreadable")")
    }
  }
  ...
  func snapshot(to file: URL, then done: @escaping () -> Void) {
    view.takeSnapshot(with: nil) { image, _ in
      ...
      NSLog("deep-reef: wrote \(file.path)")
    }
  }
}
...
final class Controller: NSObject, NSApplicationDelegate, NSMenuDelegate {
  ...
  func applicationShouldTerminateAfterLastWindowClosed(_ sender: NSApplication) -> Bool { previewMode }
  ...
  func applicationDidFinishLaunching(_ note: Notification) {
    ...
    // `kill -USR1` writes what the first screen is showing to /tmp/deep-reef.png.
  ...
  private func snapshot() {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 4) { [weak self] in
      first.probe()
      first.snapshot(to: URL(fileURLWithPath: "/tmp/deep-reef.png")) {
        self?.applyRate()
      }
    }
  }
  ...
  private func build() {
    ...
    screens = (previewMode ? Array(NSScreen.screens.prefix(1)) : NSScreen.screens).map { Wallpaper(screen: $0, root: root, habitat: habitat) }
    applyRate()
  }
  ...
  func applyRate() {
    ...
    let blockers = (still || previewMode) ? [] : windowBlockers()
    applied = 0
    var changed = false
    for (index, screen) in screens.enumerated() {
      screen.setPaused(stopped)
  ...
  private func addMenu() {
    ...
    let symbol = NSImage(systemSymbolName: "fish", accessibilityDescription: "Deep Reef")
    symbol?.isTemplate = true
    item.button?.image = symbol
    if symbol == nil { item.button?.title = "Deep Reef" }
    item.button?.toolTip = "Deep Reef · \(habitat.title)"
    ...
    let lights = NSMenu(title: "灯光")
    for (label, value) in [("深蓝展缸", "blue"), ("月光", "moon"), ("清透日光", "day")] {
      let item = NSMenuItem(title: label, action: #selector(selectLight), keyEquivalent: "")
      item.target = self; item.representedObject = value; lights.addItem(item)
    }
    let lightMenu = NSMenuItem(title: "灯光", action: nil, keyEquivalent: "")
    lightMenu.submenu = lights; menu.addItem(lightMenu)
    menu.addItem(.separator())
    feed.title = "投喂"
    feed.target = self
    ...
    menu.addItem(pause)
    let reload = NSMenuItem(title: "重新加载水族箱", action: #selector(reloadScenes), keyEquivalent: "")
    reload.target = self
    menu.addItem(reload)
    menu.addItem(.separator())
    let leave = NSMenuItem(title: "退出 Deep Reef", action: #selector(quit), keyEquivalent: "q")
    leave.target = self
    menu.addItem(leave)
    item.menu = menu
    status = item
    if item.button?.window == nil || !item.isVisible {
      NSLog("deep-reef: the menu bar item did not appear")
    }
  }
  ...
  func setLight(_ value: String) {
    guard ["blue", "moon", "day"].contains(value) else { return }
    UserDefaults.standard.set(value, forKey: "light")
    for screen in screens { screen.view.evaluateJavaScript("window.habitatLight && habitatLight(\"\(value)\")") }
  }
  ...
  @objc private func selectHabitat(_ sender: NSMenuItem) {
    ...
    status?.button?.toolTip = "Deep Reef · \(chosen.title)"
    build()
  }
  ...
  func syncPages() {
    for screen in screens { screen.setPaused(stopped); screen.syncState() }
  }

  @objc private func reloadScenes() {
    for screen in screens { screen.reload() }
  }
